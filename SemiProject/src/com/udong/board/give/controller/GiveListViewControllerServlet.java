 ...
 */
@WebServlet("/giveList.bo")
public class GiveListViewControllerServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GiveListViewControllerServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new GiveBoardService().giveSelectListCount();
		...
		ArrayList<GiveBoard> list = new GiveBoardService().selectGiveList(pi);

		request.setAttribute("gblist", list);
		request.setAttribute("pi",pi);
		request.getRequestDispatcher("views/board/give/giveListView.jsp").forward(request, response);
	}
