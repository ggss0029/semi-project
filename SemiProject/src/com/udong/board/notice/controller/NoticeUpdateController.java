...
public class NoticeUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("views/board/notice/noticeUpdateView.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result>0) { //수정한 글번호를 이용해서 상세보기 페이지 보내기
			...
			response.sendRedirect(request.getContextPath()+"/noticeList.bo?bno="+noticeNo);
		}else {
